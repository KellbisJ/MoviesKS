  ...
  const StarRating = ({ rating }: { rating?: number }) => {
    ...
    return (
      <div className="flex items-center">
        ...
        <span className="ml-2 text-sm font-medium text-gray-500 dark:text-stone-100">
          {rating}
        </span>
      </div>
    );
      ...
      {isLoading ? (
        <div className="flex flex-col gap-8 animate-pulse my-16">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="bg-white dark:bg-gray-600 rounded-xl shadow-sm p-6 mt-18">
          ...
          <div className="space-y-6 w-full max-w-3xl">
            {mediaReviews.map((review) => (
              <div
                ...
                <div className="mt-4">
                  <p className="text-gray-700 dark:text-stone-100 whitespace-pre-line wrap-break-word">
                    {review.content}
                  </p>
                </div>
