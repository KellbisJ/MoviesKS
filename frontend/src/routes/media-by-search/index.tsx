...
const MediaBySearch = (): React.JSX.Element => {
  ...
  useEffect(() => {
    ...
    if (mediaType && querySearch) {
      ...
      async function fetchMedia() {
        const mediaData = await getMediaBySearch(
  ...
  const fetchMoreMedia = async () => {
    setLoading(true);
    const nextMedia = await getMediaBySearch(
      mediaType as MediaTypeT,
      querySearch,
      page
    );
    ...
    if (nextMediaData && nextMediaData.length > 0) {
      setMoreMedia((prevMedia) => {
        const mediaIds = new Set(
          [...media, ...prevMedia].map((media) => media.id)
        );
        const uniqueNextMedia = nextMediaData.filter(
          (media): media is MovieInterface | TVInterface =>
            !mediaIds.has(media.id)
        );
        return [...prevMedia, ...uniqueNextMedia] as
          | MovieInterface[]
          | TVInterface[];
      });
